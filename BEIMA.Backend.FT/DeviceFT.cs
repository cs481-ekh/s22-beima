...
namespace BEIMA.Backend.FT
{
    ...
    public class DeviceFT : FunctionalTestBase
    {
        ...
        [Test]
        public async Task DeviceNotInDatabase_AddDevice_EmptyFileAndPhoto_CreatesNewDeviceWithFileNoPhoto()
        {
            // ARRANGE
            var device = new Device
            {
            ...
            string responseId;
            using (var fileStream = new MemoryStream(TestObjects._emptyFileBytes))
            using (var photoStream = new MemoryStream(TestObjects._emptyFileBytes))
            {
                files.Add(new FormFile(fileStream, 0, fileStream.Length, "files", fileName));
                files.Add(new FormFile(photoStream, 0, photoStream.Length, "photo", photoName));
            ...
            Assert.That(getDevice.Photo?.FileName, Is.Null);
            Assert.That(getDevice.Photo?.FileUid, Is.Null);
        ...
        [Test]
        public async Task DeviceInDatabase_UpdateDevice_EmptyFileAndPhoto_ReturnsUpdatedDeviceWithFileNoPhoto()
        {
            // ARRANGE
            var origDevice = new Device
            {
            ...
            Assert.That(updatedDevice.Photo?.FileName, Is.Null);
            Assert.That(updatedDevice.Photo?.FileUid, Is.Null);
            Assert.That(updatedDevice.Photo?.FileUrl, Is.Null);
